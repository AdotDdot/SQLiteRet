...
class RecordRetriever:
    ...
    def pl_decode_file(self, serial):
        ...
        #serial N>=12 and even: size (N-12)/2, type BLOB
        elif serial >=12 and not serial%2:
            return binascii.hexlify(self.file.read(int((serial-12)/2)))

        #serial N>=13 and odd: size (N-13)/2, type str
        elif serial >=13 and serial%2:
            strsize = int((serial-13)/2)
            return struct.unpack(str(strsize)+'s', self.file.read(strsize))[0]
    ...
    def intact_rows_bruteforce(self, offset, end_offset, table_rootno):
        ...
        while offset <= end_offset:
            ...
            try: pl_len, rowid, hd_len = self.vr.n_varints_file(3)
            except:
                offset += 1
                continue
            ...
            try:
                serials = self.vr.n_varints_file(column_no)
